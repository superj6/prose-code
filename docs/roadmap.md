...
| **2 — Robustness** (in progress) | Feedback tracking ✅, eval harness v1 ✅, conflict policy in prompt ✅, streaming preview + `reasoning_effort` ✅, cache-oriented prompt layout + `prompt_cache_key` ✅; **todo**: verifiers (`verify/`), windowed context for long files, structural (symbol-path) block keys, `extension/test` integration tests, 100-item hand-checked eval set | Baseline metrics table for `gpt-5.6-luna`; 20 min of real use loses no user text. |
