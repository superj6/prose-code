...
| **3 — Data** (pipeline done, corpus pending) | `records.py`, `seed_corpus.py` (local repos), `perturb.py` (propose edit → production `Engine.sync` → labelled record), `interactions_export.py` (real logs + feedback), `dataset.py` (renders with the production prompt builder) ✅; **todo**: run at scale on a seed corpus (Modal `gen_data` job), git-mined edits, HF seeds, freeze `test.jsonl` | ≥ 30k examples across 5 languages on the Modal volume; `test.jsonl` frozen. |
